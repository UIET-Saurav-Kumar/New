...
const fetchProducts = async ({ queryKey }: QueryParamsType) => {
  ...
  const url = `${API_ENDPOINTS.PRODUCT_OFFERS}?search=${searchString}&searchJoin=and&limit=${limit}&page=${page}&orderBy=${orderBy}&sortedBy=${sortedBy}`;
  const {
    data: { data, ...rest },
...
const useProductsQuery = (
  ...
  return useQuery<any, Error>([API_ENDPOINTS.PRODUCT_OFFERS, params], fetchProducts, {
    ...options,

...
export default function BroadbandForm({click,variant } :any) {
    ...
    const[broadbandInfo,setBroadbandInfo]= useState(null);
    ...
    const[broadbandDetails,setBroadbandDetails] = useState(null);

    const[key1, setKey1] = useState('');
    const[key2,setKey2] = useState('');
    const[key3,setKey3] = useState('')

    const fetchBroadbandInfo = async (data:any) => {
        setLoading(true)

        console.log('biller',data)
  
        const { data:response } = await http.get(`${url}/${API_ENDPOINTS.BROADBAND_INFO}?biller_id=${billerId}&category=${category}`);
        setLoading(false)
         setBroadbandInfo(response)
         
        console.log('broadbandInfo',broadbandInfo,response);
        return response;

      };

      const { mutate: mutateBroadband } = useMutation(fetchBroadbandInfo, {
        ...
        <div className='grid grid-cols-1 lg:flex space-y-3 lg:space-y-0 lg:space-x-20 px-6 justify-evenly w-full py-3 items-center bg-gray-200'>
            ...
            { loading ? 
                 ...
                 style={{width:"30px",height:"30px"}}/>
                    : 
                   broadbandInfo?.data?.map((field,index)=>
                    <div className='flex-1 '> 
                       ...
                       />
                       <h1 className='text-red-600 text-xs'>{broadbandInfo?.status_code == 500 ? broadbandInfo?.status_msg : null}</h1>

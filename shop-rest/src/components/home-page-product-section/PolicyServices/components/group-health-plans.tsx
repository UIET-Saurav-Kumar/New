...
export default function GroupHealthPlans({click,view,width,height,label,...props}:any) {

  console.log('dth view',view);

  return (

        <Link href='/group-health-insurance'>
          <div onClick={view}  
          className ={`text-center active:border-gray-400 w-full cursor-pointer border rounded-lg text-center' } ${props?.cn}`}>
              
              <Image priority={true} src='/insurance/group.png' 
                layout='intrinsic'
                objectFit='contain'
                width={width}
                height={height}

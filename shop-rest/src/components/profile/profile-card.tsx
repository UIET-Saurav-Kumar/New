...
type ShopProfileCardProps = {
  ...
  const ShopProfileCard: React.FC<ShopProfileCardProps> = ({
    ...
    const imageCheck = (logo: any , record:any, imgsize:any, imgDim:any, classname: string) => {
      ...
      return (check ?    < Image priority={true}     loading='eager' quality='40' src={logo} alt={record?.name} className={classname} width={300} height={300} />:<Avatar name={record?.name} size={imgsize} round={imgDim} maxInitials={2} />);
    }
